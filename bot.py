...
EXTREME_POOR_BALANCE_THRESHOLD: int = 1000
...
async def update_balance(user_id: int, amount: int, txn_type: str, description: str):
    ...
    # Доп. событие «нищета» при падении баланса < 1000 (отдельный антиспам-таймер).
    if (
        amount < 0
        and new_balance > 0
        ...
        if not last or (now - last).total_seconds() >= POOR_ALERT_COOLDOWN_SEC:
            await post_chronicle(
                f"🪙 Игрок {player_name} скатился в нищету. Баланс: {format_money(new_balance)}."
            )
            async with aiosqlite.connect(DB_NAME) as db2:
                await db2.execute(
