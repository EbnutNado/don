...
async def get_reputation_percent(user_id: int, user_row: Optional[Dict[str, Any]] = None) -> float:
    """
    Единый и честный рейтинг 0..100:
    - текущая шкала оценок: 1..3 звезды;
    - legacy 1..5 автоматически нормализуем в 1..3 при чтении истории.
    """
    async with aiosqlite.connect(DB_NAME) as db:
        cur = await db.execute(
            """
            SELECT AVG(
                CASE
                    WHEN stars BETWEEN 1 AND 3 THEN CAST(stars AS REAL)
                    WHEN stars BETWEEN 4 AND 5 THEN 1.0 + (CAST(stars AS REAL) - 1.0) * 0.5
                    ELSE NULL
                END
            )
        ...
        avg_stars = (await cur.fetchone())[0]

    if avg_stars is not None:
        # 1* -> 0%, 2* -> 50%, 3* -> 100%
        pct = ((float(avg_stars) - 1.0) / 2.0) * 100.0
        return max(0.0, min(100.0, pct))

    # Fallback для старых данных, если событий ещё нет.
    user = user_row or await get_user(user_id) or {}
    pts = float(user.get("rep_points") or 0)
    votes = float(user.get("rep_votes") or 0)
    if votes <= 0:
        return 50.0
    return max(0.0, min(100.0, pts / votes))


async def add_reputation_vote(from_uid: int, to_uid: int, stars: int, context: str = "") -> None:
    stars = max(1, min(3, int(stars)))
    # 1..3 звезды -> 0/50/100 очков на голос.
    points = int(round(((stars - 1) / 2.0) * 100.0))
    async with aiosqlite.connect(DB_NAME) as db:
        await db.execute(
