...
async def handle_referrals_menu(message: Message):
    ...
    bot_info = await bot.get_me()
    bot_username = bot_info.username
    # Некоторые окружения возвращают username без "_" (например: terekonik22bot),
    # а в ссылках должен быть корректный Telegram username.
    if bot_username == "terekonik22bot":
        bot_username = "terekonik22_bot"
    if not bot_username:
    ...
    lines = [
        ...
        "",
        "Доступно:",
        "Новичок (активный): +5 000 ₽ после 5+ действий (начисляется сразу).",
    ]
    ...
    while True:
        try:
            ...
            for row in pending:
                ...
                # Активность новичка: 5+ любых действий (кроме регистрации).
                async with aiosqlite.connect(DB_NAME) as db:
                    cur = await db.execute(
                        "SELECT COUNT(*) FROM transactions WHERE user_id = ? AND type != 'registration'",
                        (invitee_id,),
